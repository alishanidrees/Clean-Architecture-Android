...
import com.tawkeel.base.data.RuntimePermissionData
...
fun <T> Context.isServiceRunning(service: Class<T>): Boolean {
    return (getSystemService(Context.ACTIVITY_SERVICE) as ActivityManager)
        ...
        .any { className ->
            className.service.className == service.name
        }
}
...
fun Fragment.showSnackBar(
    ...
    backgroundColor: Int = R.color.app_blue,
...
//            if (destination.label == "seeker_umrah_tracking")
//                navController.popBackStack(destination.id, inclusive = true)
//        }
...
fun Fragment.getAppHeaderHeight() = resources.getDimension(com.intuit.sdp.R.dimen._70sdp).toInt()
...
fun Fragment.requestForPermission(runtimePermissionData: RuntimePermissionData) {
    (requireActivity() as HostActivity).requestForPermission(
        runtimePermissionData.permissions,
        runtimePermissionData.runtimePermissionHandler
    )
}
        ...
        (dialogInterface as? BottomSheetDialog)
            ...
            ?.let {
                ...
                it.layoutParams = it.layoutParams.apply {
                    ...
                    width = context?.getWindowWidth()?.times(90)?.div(100).default
                }
...
fun BottomSheetDialogFragment.configureBottomSheetWithPercentage(dialog: Dialog, enableDragging: Boolean = false) {
    dialog.setOnShowListener { dialogInterface ->
        (dialogInterface as? BottomSheetDialog)
            ?.findViewById<FrameLayout>(com.google.android.material.R.id.design_bottom_sheet)
            ?.let {
...
fun Fragment.lastDialogFragmentDestination() = findNavController().previousBackStackEntry

@SuppressLint("RestrictedApi")
fun Fragment.lastFragmentDestination() = findNavController().previousBackStackEntry
