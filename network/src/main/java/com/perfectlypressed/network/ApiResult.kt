...
sealed class ApiResult<out T> {
  ...
  //  data class SuccessNafath(val data: Result) : ApiResult<NafathResponse>()

    companion object {
        fun technicalFailure(): Failure =
            Failure(ErrorResponse(ApiErrors.EXCEPTION_MESSAGE, ApiErrors.EXCEPTION_CODE))

        fun timeoutFailure(exception: Exception): Failure =
            Failure(ErrorResponse(ApiErrors.TIMEOUT_MESSAGE, ApiErrors.EXCEPTION_CODE))
